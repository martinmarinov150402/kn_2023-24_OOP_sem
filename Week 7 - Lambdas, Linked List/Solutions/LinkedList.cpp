...
{
    return getNodeOnIndex(idx)->getData();
}
...
int main()
{
    ...
    LinkedList<int> evens = a.filter([](int el)->bool{return el%2 == 0;});
    for(int i = 0; i < 2; i++)
    {
        std::cout << evens.getAt(i) << " ";
    }
    std::cout << std::endl;

    LinkedList<int>multByTwo = a.map([](int el)->int{return el * 2;});
    for(int i = 0; i < 5; i++)
    {
        std::cout << multByTwo.getAt(i) << " ";
    }
}
